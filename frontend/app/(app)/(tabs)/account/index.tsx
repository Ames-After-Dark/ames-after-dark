...
import {
    getUserById, getUserProfileByAuth, getPendingFriendRequests,
    getRecommendedFriends, acceptFriendRequest, declineFriendRequest,
    sendFriendRequest, removeFriend, PendingFriendRequest
} from '@/services/userService';

// Types & Utils
import { Friend, UserProfile } from '@/types/types';
...
import { FriendsModal } from '@/components/account/friends-modal';
import { PendingRequestsModal } from '@/components/account/pending-friend-requests-modal';
...
export default function AccountScreen() {
    ...
    // Constants
    const targetId = id ? Number(id) : userStatus?.userId;
    const isOwnProfile = !id || Number(id) === userStatus?.userId;
    ...
    const [activeModal, setActiveModal] = useState<'none' | 'friends' | 'pending'>('none');

    // Friends Hook
    const { friends, refetch: refetchFriends } = useFriends(targetId || 0);
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const rawData = isOwnProfile
                ? await getUserProfileByAuth(token!)
                : await getUserById(targetId);

            setUser(normalizeUserData(rawData));

            if (isOwnProfile) {
                const [pending, recs] = await Promise.all([
                    getPendingFriendRequests(targetId),
                    getRecommendedFriends(targetId, 5)
                ]);
    ...
    }, [targetId, isOwnProfile, getAccessToken]);
    ...
    const handleRefresh = async () => {
        setRefreshing(true);
        await Promise.all([fetchData(), refetchFriends()]);
        setRefreshing(false);
    };

    // --- Action Handlers ---
    const handleFriendAction = async (request: PendingFriendRequest, action: 'accept' | 'decline') => {
        const friendId = request.user_id_1 === targetId ? request.user_id_2 : request.user_id_1;
        setActionLoadingId(friendId);
        try {
            if (action === 'accept') await acceptFriendRequest(targetId!, friendId);
            else await declineFriendRequest(targetId!, friendId);

            setPendingRequests(prev => prev.filter(r => r.user_id_1 !== friendId && r.user_id_2 !== friendId));
            refetchFriends();
        } finally {
    ...
    const handleRemoveFriend = async (friendId: number) => {
        setActionLoadingId(friendId);
        try {
            await removeFriend(targetId!, friendId);
            refetchFriends();
        } finally {
            setActionLoadingId(null);
        }
    ...
    return (
        <View style={{ flex: 1, backgroundColor: Theme.dark.background }}>
            <ScrollView
                ...
                refreshControl={
                    <RefreshControl
                        ...
                        tintColor={Theme.dark.secondary}
                    />
                }
            >
                ...
                <View style={styles.statsRow}>
                    ...
                    {isOwnProfile && (
                        <TouchableOpacity style={styles.statButton} onPress={() => setActiveModal('pending')}>
                ...
                <View style={styles.gridRow}>
                    <FeatureCard title="fav. drink" style={{ flex: 1 }}>
                        <View style={styles.placeholderBox}>
                            <FontAwesome name="glass" size={24} color={Theme.dark.secondary} />
                        </View>
                    </FeatureCard>

                    <FeatureCard title="streak" style={{ flex: 1 }}>
                        <Text style={styles.streakEmoji}>🔥 {user?.streak || 0}</Text>
                        <Text style={styles.subLabel}>weekends out in a row</Text>
                    </FeatureCard>
                </View>

                <FeatureCard title="fav. bar w/ official photo" style={styles.largeCard}>
                    <View style={styles.largePlaceholder}>
                        <FontAwesome name="map-marker" size={40} color={Theme.dark.muted} />
                        <Text style={styles.placeholderText}>No favorite bar set</Text>
                    </View>
                </FeatureCard>
            </ScrollView>
            ...
            <FriendsModal
                ...
                onAddRecommended={async (id) => {
                    await sendFriendRequest(targetId!, id);
                    fetchData();
                }}
            ...
            <PendingRequestsModal
                visible={activeModal === 'pending'}
                onClose={() => setActiveModal('none')}
                requests={pendingRequests}
                currentUserId={targetId || 0}
...
const styles = StyleSheet.create({
    ...
    streakEmoji: { fontSize: 32, fontWeight: 'bold', color: Theme.dark.tertiary },
    subLabel: { color: Theme.container.inactiveText, fontSize: 10, textAlign: 'center' },
    ...
    placeholderText: { color: Theme.dark.muted, fontSize: 12 }
});
...
//     // Determine the profile we are targeting
//     const targetId = id ? Number(id) : userStatus?.userId;
//     const isOwnProfile = !id || Number(id) === userStatus?.userId;
...
//     const { friends, loading, error, refetch } = useFriends(targetId || 0);
//     const [pendingRequests, setPendingRequests] = useState<PendingFriendRequest[]>([]);
...
//             if (isOwnProfile) {
//                 userData = await getUserProfileByAuth(accessToken!);
//             } else {
//                 const rawFriendData = await getUserById(targetId);

//                 // MAP THE DATA HERE to match your Profile type
//                 // This ensures "id" exists even if the API returns "user_id"
//                 userData = {
//                     ...rawFriendData,
//                     id: rawFriendData.id || rawFriendData.user_id,
//                     name: rawFriendData.name || rawFriendData.full_name,
//                     username: rawFriendData.username,
//                     avatar: rawFriendData.avatar || rawFriendData.profile_pic_url,
//                     bio: rawFriendData.bio,
//                     streak: rawFriendData.streak || 0
//                 };
//             }

//             console.log("Setting user to:", userData.username);
//             setUser(userData);
//         } catch (err) {
//             console.error('Fetch error:', err);
//         } finally {
...
//     useEffect(() => {
//         // 1. Wipe the current data immediately
//         setUser(null);

//         // 2. Fetch the new person
//         fetchUser();

//         // 3. Optional: Refetch the friends list for the new person
//         refetch();
//     }, [id]); // This fires every time the URL changes (e.g. from /account to /account/3)
...
//                     {/* ONLY show gear if it is your own profile */}
//                     {isOwnProfile && (
//                         <TouchableOpacity onPress={() => router.push('/(app)/(tabs)/account/settings')}>
//                             <FontAwesome name="gear" size={24} color={Theme.container.inactiveText} />
//                         </TouchableOpacity>
...
//                         </TouchableOpacity>
//                     ) : (
//                         /* Show an "Add Friend" button if viewing someone else */
//                         <TouchableOpacity
//                             style={[styles.statButton, { backgroundColor: Theme.dark.secondary }]}
//                             onPress={() => handleAddRecommendedFriend(targetId!)}
//                         >
//                             <FontAwesome name="user-plus" size={20} color="white" />
//                             <Text style={[styles.statLabelSmall, { color: 'white' }]}>Add Friend</Text>
//                         </TouchableOpacity>
//                     )}
//                 </View>
